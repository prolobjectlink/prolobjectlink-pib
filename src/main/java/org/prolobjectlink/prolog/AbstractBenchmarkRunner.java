...
public abstract class AbstractBenchmarkRunner implements BenchmarkRunner {
	...
	public final void output(Collection<RunResult> results) {
		...
		try {
			...
			File outfile = null;
			if (!dist.getCanonicalPath().contains("prolobjectlink-pib")) {
				File out = new File(dist.getCanonicalPath() + File.separator + "out");
				out.mkdirs();
				outfile = new File(out.getCanonicalPath() + File.separator + name);
			} else {
				outfile = new File("out" + File.separator + name);
			}
